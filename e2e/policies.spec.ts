...
test('UI075-01a managed list/detail, valid create/edit review, impact, and protected deletion', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Delete policy' })).toBeDisabled()
...
test('UI075-01b malformed/unsupported drafts and stale 409 preserve exact draft then recover', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Review change' }).click()
  await dialog.getByRole('button', { name: 'Confirm change' }).click()
  await expect(page.getByRole('region', { name: 'Managed policy detail' })).toContainText('draft-preserved:@/#?')
  expect(state.requests.filter(request => request.method === 'PUT')).toHaveLength(2)
  await assertBrowserBoundary(page)
  state.verify()
})

test('policy popups keep actions in view over long lists and preserve drafts and row focus', async ({ page }, testInfo) => {
  const state = await managedFixture(page)
  await page.route('**/admin/ui/policies?limit=100', route => route.fulfill({ json: {
    items: [editableSummary, ...Array.from({ length: 60 }, (_, index) => ({ ...editableSummary, id: `00000000-0000-4000-8000-${String(index + 500).padStart(12, '0')}`, name: `Policy-${index}` }))],
    next_after_id: null, default_page_size: 100, max_page_size: 200,
  } satisfies PolicyPage }))
  await page.route(`**/admin/ui/policies/${editableId}`, route => route.fulfill({ json: detail(editableSummary, { Version: '2012-10-17', Statement: Array.from({ length: 50 }, (_, index) => ({ ...allowDocument.Statement[0], Sid: `Statement${index}` })) }) }))
  await openPolicies(page)
  const opener = page.getByRole('button', { name: 'View ReportsRead' })
  ...
  const edit = popup.getByRole('button', { name: 'Edit policy' })
  ...
  await page.screenshot({ path: testInfo.outputPath('policy-popup.png') })
  await edit.click()
  popup = page.getByRole('dialog', { name: 'Edit ReportsRead' })
  await expect(popup).toHaveCount(1)
  await popup.getByLabel('Description').fill('Unsaved description')
  await page.keyboard.press('Escape')
  await expect(popup.getByRole('alert')).toContainText('Discard unsaved changes?')
  await popup.getByRole('button', { name: 'Keep editing' }).click()
  await expect(popup.getByLabel('Description')).toHaveValue('Unsaved description')
  await expect(popup.getByRole('button', { name: 'Review change' })).toBeInViewport({ ratio: 1 })
  await page.screenshot({ path: testInfo.outputPath('policy-editor-popup.png') })
  await popup.getByRole('button', { name: 'Cancel', exact: true }).click()
  await expect(page.getByRole('dialog', { name: 'ReportsRead', exact: true })).toBeInViewport({ ratio: 1 })
  await page.keyboard.press('Escape')
...
test('UI075-03 direct/virtual owner scopes isolate aliases and reviewed mutations stay metadata-only', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Edit policy' })).toBeInViewport({ ratio: 1 })
  await page.getByRole('button', { name: 'Edit policy' }).click()
  ...
  await expect(page.getByRole('region', { name: 'Bucket policy detail' })).toContainText('DenyDelete')
  await page.getByRole('button', { name: 'Close dialog' }).click()
