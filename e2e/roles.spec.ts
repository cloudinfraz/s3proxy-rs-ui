...
async function fixture(page: Page, empty = false) {
  ...
  await page.route('**/*', async route => {
    ...
    if (!path.startsWith('/admin/ui/roles') && path !== '/admin/ui/role-policies') {
      if (['/admin/session', '/admin/ui/identities', '/admin/ui/identity-pages', '/admin/capabilities'].includes(path) || /^\/admin\/ui\/identities\/[^/]+$/.test(path)) return route.fallback()
...
function operationDialog(page: Page) {
  return page.getByRole('dialog').filter({ hasNot: page.getByRole('region', { name: 'Role details' }) }).or(page.getByRole('alertdialog'))
}

async function roleAction(page: Page, name: string) {
...
test('UI073-03 retirement is explicit, bounded, rechecks re-enable and gates deletion', async ({ page }, testInfo) => {
  ...
  await openRole(page)
  await page.getByRole('button', { name: 'Actions', exact: true }).click()
  await expect(page.getByRole('menuitem', { name: 'Retire sessions', exact: true })).toBeDisabled()
  await expect(page.getByRole('menuitem', { name: 'Delete role', exact: true })).toBeDisabled()
  await page.keyboard.press('Escape')
  const dialog = operationDialog(page)
  async function disable() {
    await roleAction(page, 'Disable role')
    await dialog.getByRole('button', { name: 'Review change', exact: true }).click()
    await dialog.getByRole('button', { name: /^(Disable|Delete) Reports$/, exact: true }).click()
    await expect(dialog).toHaveCount(0)
  }
  await disable()
  await roleAction(page, 'Retire sessions')
  ...
  await page.getByRole('button', { name: 'Refresh', exact: true }).click()
  await expect(page.getByRole('region', { name: 'Role details' }).locator('dt').filter({ hasText: /^Status$/ }).locator('+ dd')).toHaveText('Enabled')
  await disable()
  await roleAction(page, 'Retire sessions')
  await dialog.getByLabel('Maximum rows this batch').fill('2')
  await dialog.getByRole('button', { name: 'Review change', exact: true }).click()
  ...
  await roleAction(page, 'Delete role')
  await dialog.getByRole('button', { name: 'Review change', exact: true }).click()
  await dialog.getByRole('button', { name: /^(Disable|Delete) Reports$/, exact: true }).click()
...
test('role popup keeps operations in view over long lists and restores keyboard focus', async ({ page }, testInfo) => {
  const state = await fixture(page)
  await page.route('**/admin/ui/roles?limit=100', route => route.fulfill({ json: {
    items: [initial.role, ...Array.from({ length: 60 }, (_, index) => ({ ...initial.role, id: `00000000-0000-4000-8000-${String(index + 500).padStart(12, '0')}`, role_name: `Role-${index}` }))], next_after_id: null, limits,
  } satisfies components['schemas']['AdminIamRolePage'] }))
  await page.route(`**/admin/ui/roles/${owner}`, route => route.fulfill({ json: { ...initial, trust: { statements: Array.from({ length: 50 }, () => initial.trust.statements[0]) } } satisfies Detail }))
  await page.goto('/admin/ui/iam-roles')
  const opener = page.getByRole('button', { name: 'View Reports', exact: true })
  await opener.scrollIntoViewIfNeeded()
  const scrollBefore = await page.evaluate(() => window.scrollY)
  await opener.click()
  const popup = page.getByRole('dialog', { name: 'Reports', exact: true })
  await expect(popup).toBeInViewport({ ratio: 1 })
  await expect(popup.getByRole('button', { name: 'Change duration' })).toBeInViewport({ ratio: 1 })
  await expect(popup.getByRole('button', { name: 'Actions', exact: true })).toBeInViewport({ ratio: 1 })
  expect(await popup.locator('.dialog-body').evaluate(element => element.scrollHeight > element.clientHeight)).toBe(true)
  await page.screenshot({ path: testInfo.outputPath('role-popup.png') })
  await popup.getByRole('button', { name: 'Actions', exact: true }).focus()
  await page.keyboard.press('Enter')
  await expect(page.getByRole('menu')).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(popup.getByRole('button', { name: 'Actions', exact: true })).toBeFocused()
  await popup.getByRole('button', { name: 'Change duration' }).click()
  await expect(page.getByRole('dialog')).toHaveCount(1)
  const editor = operationDialog(page)
  await editor.getByLabel('Maximum duration (seconds)').fill('7200')
  await page.keyboard.press('Escape')
  await expect(editor.getByRole('alert')).toContainText('Discard unsaved changes?')
  await editor.getByRole('button', { name: 'Keep editing' }).click()
  await expect(editor.getByLabel('Maximum duration (seconds)')).toHaveValue('7200')
