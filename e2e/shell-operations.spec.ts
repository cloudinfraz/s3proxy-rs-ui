...
test('UI069-03 keyboard navigation and unavailable capability states', async ({ page }) => {
  ...
  await page.goto('/admin/ui/')
  await expect(page.getByText('No configuration issues found.')).toBeVisible()
...
test('UI069-04 one-time key dismissal, toggle and delete preserve metadata only', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog').or(page.getByRole('alertdialog'))).toContainText(synthetic)
  ...
  await page.getByRole('alertdialog').getByRole('button', { name: 'Disable new-admin', exact: true }).click()
  ...
  await page.getByRole('alertdialog').getByRole('button', { name: 'Enable new-admin', exact: true }).click()
  ...
  await page.getByRole('alertdialog').getByRole('button', { name: 'Delete new-admin', exact: true }).click()
...
for (const transition of ['route transition', 'blocked dismissal then history navigation', 'logout'] as const) {
...
test('UI069-05 audit pages remain inside the selected recent event window', async ({ page }) => {
  ...
  await expect(page.locator('tbody tr, .mobile-records > dl')).toHaveCount(25)
