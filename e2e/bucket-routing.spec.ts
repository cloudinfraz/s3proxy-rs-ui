...
test('UI072-05 dotted aliases stay local and a corrected alias submits unchanged', async ({ page }, testInfo) => {
  ...
  await expect(dialog).toHaveCount(0)
  await expect(page.getByRole('cell', { name: 'reports-new', exact: true })).toBeVisible()
  await expect(page.getByRole('cell', { name: 'physical-container', exact: true })).toBeVisible()
...
test('UI072-01 remaps, inherits and toggles only the selected owned mapping', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: 'Review changes', exact: true }).click()
  await expect(page.getByRole('dialog')).toContainText('fixtureaccount / fixturecontainer')
  await expect(page.getByRole('dialog')).toContainText('otheraccount / remapped-container')
  ...
  await expect(page.getByRole('dialog')).toContainText('Identity default: fixtureaccount / remapped-container')
...
test('UI072-02 unavailable selections and stale review cannot silently persist', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Confirm change', exact: true }).click()
  await expect(page.getByRole('dialog')).toContainText('Mapping review expired')
...
test('UI072-03 cancelled and failed removal preserve the mapping and other owner', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog')).toContainText('Azure containers, blobs and native versions are retained')
