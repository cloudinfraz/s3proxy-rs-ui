...
- Any present marker value, including a malformed value, remains blocked until
  authoritative verification resolves it.
- A storage read or write failure does not unblock the current document.
...
5. Reload a malformed marker and verify it remains blocked without affecting a
   second tab.
6. Verify browser storage and displayed errors contain no secret material or
...
- `DELETE /admin/session` requires that CSRF token and returns `204` only after
  server-side revocation is complete.# Logout revocation recovery
