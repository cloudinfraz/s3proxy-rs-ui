    ...
    {operation && operation !== 'delete' && <Modal title={title} description={operation === 'edit' ? 'Container, backend, prefix, and status may change. Alias and identity ownership remain fixed; changing ownership requires a new mapping.' : 'Routing metadata only. Changes do not move data or verify Azure access.'} onClose={dismiss} pending={pending}>
      ...
      {stale ? <div className="dialog-actions"><button disabled={pending} onClick={dismiss}>Cancel</button><button className="primary" disabled={pending} onClick={() => { if (selected) void open('edit', selected.id); else { dismiss(); void invalidateControl(client) } }}>Reload and review</button></div>
        : (operation === 'create' || (operation === 'edit' && selected)) && !review ? <form className="operation-form" onSubmit={event => { void prepare(event) }}>
          ...
          <div className="dialog-actions"><button type="button" disabled={pending} onClick={dismiss}>Cancel</button><button className="primary" disabled={pending || !context}>{pending ? 'Loading review...' : 'Review changes'}</button></div>
              ...
              : selected ? <><dl className="mapping-detail"><dt>Mapping ID</dt><dd>{selected.id}</dd><dt>S3 bucket</dt><dd>{selected.virtual_bucket_name}</dd><dt>Owner ID</dt><dd>{selected.credential_id}</dd><dt>Effective target</dt><dd>{target(mappingDraft(selected), context)}</dd><dt>Identity default backend</dt><dd>{selected.credential_default_backend_id ?? 'Legacy account'}</dd><dt>Endpoint prefix</dt><dd>{selected.endpoint_prefix ?? 'None'}</dd><dt>Status</dt><dd>{selected.enabled ? 'Enabled' : 'Disabled'}</dd></dl><div className="dialog-actions"><button onClick={dismiss}>Close</button></div></> : <p role="status">{pending ? 'Loading mapping...' : 'Mapping unavailable'}</p>}
    </Modal>}
            {operation === 'delete' && !selected && <Modal title={title} description="Loads authoritative mapping impact before removal." onClose={dismiss} pending={pending}>{error && <ErrorBanner error={error} />}<p role="status">{pending ? 'Loading mapping...' : 'Mapping unavailable'}</p></Modal>}
            {operation === 'delete' && selected && <DestructiveDialog title="Remove bucket routing" description="Removes only this owned mapping and its scoped policy. Azure containers, blobs and native versions are retained." confirmLabel={`Remove ${selected.virtual_bucket_name}`} pending={pending} onClose={dismiss} onConfirm={() => { void persist() }}><dl className="mapping-detail"><dt>S3 bucket</dt><dd>{selected.virtual_bucket_name}</dd><dt>Effective target</dt><dd>{target(mappingDraft(selected), context)}</dd><dt>Owner ID</dt><dd>{selected.credential_id}</dd></dl>{error && <ErrorBanner error={error} />}</DestructiveDialog>}
  </Page>
}