...
import { KeyRound, Pencil, Plus, RotateCw, Trash2 } from 'lucide-react'
...
export default function IdentitiesPage() {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await api<Schema['CredentialCreatedResponse']>('/admin/credentials', {
        method: 'POST',
        body: JSON.stringify(createIdentityPayload({
  ...
  return <Page title="S3 identities" subtitle="Signing identities and their control-plane dependencies" action={<div className="page-actions"><RefreshButton pending={identities.isFetching} refresh={() => { void identities.refetch() }} /><button className="primary" onClick={() => { dismiss(); setCreating(true) }}><Plus size={17} /> Create identity</button></div>}>
    {identities.isError && <ErrorBanner error={identities.error} retry={() => { void identities.refetch() }} />}
    {(!identities.isError || identities.data) && <DataTable rows={identities.data ?? []} rowKey={identity => identity.credential_id ?? identity.s3_access_key} loading={identities.isPending} columns={[
