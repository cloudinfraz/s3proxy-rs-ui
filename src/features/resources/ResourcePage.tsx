...
const resources: Record<string, ResourceConfig> = {
  ...
  backends: { resource: 'backends', path: '/admin/backends', title: 'Azure backends', identityKey: 'name', defaults: { auth_mode: 'managed_identity', user_delegation_sas_enabled: true, enabled: true, secret_ref: null, region_label: null }, columns: [{ key: 'name', label: 'Name' }, { key: 'azure_account', label: 'Azure account' }, { key: 'auth_mode', label: 'Authentication' }, { key: 'enabled', label: 'Status' }], fields: [{ key: 'name', label: 'Name', required: true }, { key: 'azure_account', label: 'Azure account', required: true }] },
...
export default function ResourcePage({ resourceName }: { resourceName: string }) {
  ...
  return <Page title={config.title} action={<button className="primary" onClick={() => { close(); setCreating(true) }}><Plus size={17} /> Add {config.title.toLowerCase()}</button>}>{query.isError && <ErrorBanner error={query.error} retry={() => { void query.refetch() }} />}{(!query.isError || query.data) && <DataTable rows={query.data ?? []} loading={query.isPending} rowKey={row => String(row.id ?? row.credential_id ?? row.name ?? row.s3_access_key)} columns={[...config.columns.map(column => ({ label: column.label, value: (row: RecordValue) => display(row[column.key]) })), { label: 'Actions', value: row => <button className="icon-button danger" aria-label="Delete record" title="Delete record" onClick={() => { close(); setDeleting(String(row[config.identityKey])) }}><Trash2 size={16} /></button> }]} />}{creating && <Modal title={`Add ${config.title.toLowerCase()}`} description={config.title} onClose={close}><form className="operation-form" onSubmit={submit}>{config.fields.map(field => <label key={field.key}>{field.label}{field.key === 'document' ? <textarea name={field.key} required rows={8} defaultValue={'{"Version":"2012-10-17","Statement":[]}'} /> : <input name={field.key} type={field.type ?? 'text'} required={field.required} />}</label>)}{error && <ErrorBanner error={error} />}<div className="dialog-actions"><button type="button" onClick={close}>Cancel</button><button className="primary" disabled={pending}>{pending ? 'Saving...' : 'Save'}</button></div></form></Modal>}{deleting !== null && <Modal title="Delete record" description={config.resource === 'buckets' ? 'Removes routing metadata only. Azure containers and data are not deleted.' : 'Removes this control-plane record and may affect dependent access.'} onClose={close}>{error && <ErrorBanner error={error} />}<div className="dialog-actions"><button onClick={close}>Cancel</button><button className="primary" disabled={pending} onClick={() => { void remove() }}>{pending ? 'Deleting...' : 'Delete'}</button></div></Modal>}</Page>
}