...
export default function BackendsPage() {
  ...
  return <Page title="Azure backends" subtitle="Managed Identity routing registrations and their control-plane references" action={<div className="page-actions"><RefreshButton pending={backends.isFetching} refresh={() => { void backends.refetch() }} /><button className="primary" disabled={!capabilities.data?.backend_routing_enabled || modes.length === 0} onClick={() => { dismiss(); setFormAuthMode(modes[0]); setCreating(true) }}><Plus size={17} /> Register backend</button></div>}>
    ...
    {action?.kind === 'detail' && <Modal title="Backend details" description={action.backend.name} onClose={dismiss}><BackendDetails backend={action.backend} /><div className="dialog-actions"><button onClick={dismiss}>Close</button></div></Modal>}
    {reviewRequired && <Modal title="Backend changed" description="Reload the current backend before reviewing another update." onClose={dismiss}>{error && <ErrorBanner error={error} />}<div className="dialog-actions"><button onClick={dismiss}>Cancel</button><button className="primary" disabled={pending} onClick={() => { void reloadForReview() }}>{pending ? 'Loading...' : 'Reload backend'}</button></div></Modal>}
  </Page>
}