...
it('proxies role metadata without intercepting the IAM roles document route', () => {
  ...
  for (const path of ['/admin/ui/roles', '/admin/ui/roles?limit=100', '/admin/ui/roles/role-id/settings', '/admin/ui/role-policies?limit=100', '/admin/ui/identity-pages?limit=100']) expect(matcher.test(path)).toBe(true)
